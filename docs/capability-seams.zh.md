...
flowchart LR
  ...
  svc_alarmQuery["ctx.alarmQuery<br/>Alarm query provider registry"]
  ...
  pkg_integration_alarm --> svc_alarmQuery
  pkg_integration_alarm_http --> svc_alarmQuery
  ...
  svc_alarmQuery --> pkg_tool_integration_alarm
...
| `ctx.alarmQuery` | `seam` | [`integration-alarm`](../packages/integration/integration-alarm) | [`integration-alarm-http`](../packages/integration/integration-alarm-http) | [`tool-integration-alarm`](../packages/integration/tool-integration-alarm) | - | 告警提供方注册到同一个 ctx.alarmQuery seam；tool-integration-alarm 负责稳定的面向模型名称。 |
